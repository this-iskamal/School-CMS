...
const uploadPath = 'api/uploads/carousel';
...
export const deleteCarousels = async (req, res, next) => {
  if (!req.user.isAdmin || req.user.id !== req.params.userId) {
    return next(
      errorHandler(403, "You are not allowed to delete this carousel")
    );
  }
...
export const updateCarousel = [
  ...
  async (req, res, next) => {
    if (!req.user.isAdmin) {
      return next(
        errorHandler(403, "You are not allowed to update this carousel")
      );
    }
    if (!req.body.description) {
      return next(errorHandler(400, "Please provide all the fields"));
